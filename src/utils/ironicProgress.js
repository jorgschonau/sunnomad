...
// group: 'early' | 'later'. Array order within each group ≈ easy → hard.
const BADGES = [
  // ── Starter-Pack ──
  {
 ...
 * Returns { badges: [{id, group, icon, nameKey, descKey, earned, descParams?}], newlyEarned: [id] }
 */
export async function resolveIronicBadges(
  ...
  return {
    badges: BADGES.map((b) => ({
      id: b.id,
      group: b.group === 'later' ? 'later' : 'early',
      icon: b.icon,
      nameKey: b.nameKey,
